...
/**
 * Class to handle {@link Command} actions and call the Mercedes Web Api.
 *
 * @author Andreas Stenlund - Initial contribution
 * @author Hilbrand Bouwkamp - Moved to separate class, general refactoring and bug fixes
 */
@NonNullByDefault
class MercedesHandleCommands {

    private final Logger logger = LoggerFactory.getLogger(MercedesHandleCommands.class);

    private final MercedesApi mercedesApi;
     ...
     *
     * @param mercedesApi The api class to use to call the mercedes api
     */
    public MercedesHandleCommands(MercedesApi mercedesApi) {
        this.mercedesApi = mercedesApi;
    }
    ...
    /**
     * Handles commands from the given Channel and calls Mercedes Web Api with the given command.
     *
     * @param channelUID Channel the command is from
     * @param command command to run
     * @param active true if current known device is the active device
     * @param deviceId Current known active Mercedes device id
        ...
        switch (channel) {
            ...
            case CHANNEL_DEVICESHUFFLE:
                if (command instanceof OnOffType) {
                    mercedesApi.setShuffleState(deviceId, (OnOffType) command);
                    commandRun = true;
                }
                break;
            case CHANNEL_TRACKREPEAT:
                if (command instanceof StringType) {
                    mercedesApi.setRepeatState(deviceId, command.toString());
                    commandRun = true;
                }
            case CHANNEL_DEVICEVOLUME:
                if (command instanceof DecimalType) {
                    ...
                    mercedesApi.setVolume(deviceId, volume.intValue());
                    commandRun = true;
                }
                break;
            ...
            case CHANNEL_PLAYLISTS:
                if (command instanceof StringType) {
                    mercedesApi.playTrack(deviceId, command.toString());
                    commandRun = true;
                }
                break;
            case CHANNEL_PLAYLISTNAME:
                if (command instanceof StringType) {
                    ...
                    playlists.stream().filter(pl -> pl.getName().equals(newName)).findFirst()
                            .ifPresent(pl -> mercedesApi.playTrack(deviceId, pl.getUri()));
                    commandRun = true;
                }
                break;
        }
    ...
    private void playDeviceId(String newDeviceId, boolean active, String currentDeviceId) {
        if (currentDeviceId.equals(newDeviceId) && active) {
            mercedesApi.play(newDeviceId);
        } else {
            mercedesApi.transferPlay(newDeviceId, true);
        }
    }
     ...
     * @param deviceId Mercedes device id the command is intended for
     * @return true if the command was done or else if no channel or command matched
     */
    private boolean handleDevicePlay(Command command, boolean active, String deviceId) {
        if (command instanceof PlayPauseType) {
            ...
            if (active || deviceId.isEmpty()) {
                if (play) {
                    mercedesApi.play(deviceId);
                } else {
                    mercedesApi.pause(deviceId);
                }
            } else {
                mercedesApi.transferPlay(deviceId, play);
            }
            return true;
        } else if (command instanceof NextPreviousType) {
            if (command == NextPreviousType.NEXT) {
                mercedesApi.next(deviceId);
            } else {
                mercedesApi.previous(deviceId);
            }
            return true;
        }
        return false;
