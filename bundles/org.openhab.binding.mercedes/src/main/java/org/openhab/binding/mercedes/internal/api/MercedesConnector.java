...
/**
 * Class to perform the actual call to the Mercedes Api, interprets the returned Http status codes, and handles the
 * error
 * codes returned by the Mercedes Web Api.
 *
 * @author Hilbrand Bouwkamp - Initial contribution
 */
@NonNullByDefault
class MercedesConnector {
    ...
    private final Logger logger = LoggerFactory.getLogger(MercedesConnector.class);
     ...
     */
    public MercedesConnector(ScheduledExecutorService scheduler, HttpClient httpClient) {
        this.scheduler = scheduler;
        this.httpClient = httpClient;
    }

    /**
     * Performs a call to the Mercedes Web Api and returns the raw response. In there are problems this method can throw
     * a Mercedes exception.
     *
    ...
    public ContentResponse request(Function<HttpClient, Request> requester, String authorization) {
        ...
        try {
            ...
            throw new MercedesException("Thread interrupted");
    ...
    /**
     * Class to handle a call to the Mercedes Web Api. In case of rate limiting or not finished jobs it will retry in a
    ...
    private class Caller {
            ...
            try {
                ...
                if (!success) {
                    if (attempts < HTTP_CLIENT_RETRY_COUNT) {
                        logger.debug("Mercedes Web API call attempt: {}", attempts);

                        scheduler.schedule(this::call, delaySeconds, TimeUnit.SECONDS);
                    } else {
                        logger.debug("Giving up on accessing Mercedes Web API. Check network connectivity!");
                        future.completeExceptionally(new MercedesException(
                                "Could not reach the Mercedes Web Api after " + attempts + " retries."));
                    }
                }
            } catch (ExecutionException e) {
        ...
        /**
         * Processes the response of the Mercedes Web Api call and handles the HTTP status codes. The method returns
         * true
         * if the response indicates a successful and false if the call should be retried. If there were other problems
         * a Mercedes exception is thrown indicating no retry should be done an the user should be informed.
         *
         * @param response the response given by the Mercedes Web Api
        ...
        private boolean processResponse(ContentResponse response) {
            ...
            switch (response.getStatus()) {
                ...
                case ACCEPTED_202:
                    logger.debug(
                            "Mercedes Web API returned code 202 - The request has been accepted for processing, but the processing has not been completed.");
                    future.complete(response);
                    success = true;
                    break;
                case BAD_REQUEST_400:
                    throw new MercedesException(processErrorState(response));
                case UNAUTHORIZED_401:
                    throw new MercedesAuthorizationException(processErrorState(response));
                case TOO_MANY_REQUESTS_429:
                    ...
                    logger.debug(
                            "Mercedes Web API returned code 429 (rate limit exceeded). Retry After {} seconds. Decrease polling interval of bridge! Going to sleep...",
                            retryAfter);
                ...
                default:
                    throw new MercedesException("Mercedes returned with error status: " + response.getStatus());
            }
            return success;
        }
         ...
         * which the error was returned by the Mercedes Web Api.
         *
         * @param response content returned by Mercedes Web Api
                ...
                if (element.isJsonObject()) {
                    ...
                    if (object.has("error") && object.get("error").isJsonObject()) {
                        ...
                        if (message.contains("expired")) {
                            throw new MercedesTokenExpiredException(message);
                        } else {
                            return message;
                        }
                        ...
                        throw new MercedesAuthorizationException(errorDescription);
                    }
                }
