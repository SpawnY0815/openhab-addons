 ...
 */
@Component(service = MercedesAuthService.class, configurationPid = "binding.mercedes.authService")
@NonNullByDefault
public class MercedesAuthService {
    ...
    private final List<MercedesAccountHandler> handlers = new ArrayList<>();
    ...
    public String authorize(String servletBaseURL, String state, String code) {
        final MercedesAccountHandler listener = getMercedesAuthListener(state);

        if (listener == null) {
            logger.debug(
                    "Mercedes redirected with state '{}' but no matching bridge was found. Possible bridge has been removed.",
                    state);
            throw new MercedesException(ERROR_UKNOWN_BRIDGE);
        } else {
     ...
     */
    public void addMercedesAccountHandler(MercedesAccountHandler listener) {
     ...
     */
    public void removeMercedesAccountHandler(MercedesAccountHandler handler) {
     ...
     */
    public List<MercedesAccountHandler> getMercedesAccountHandlers() {
        return handlers;
    }
     ...
     */
    private @Nullable MercedesAccountHandler getMercedesAuthListener(String thingUID) {
        final Optional<MercedesAccountHandler> maybeListener = handlers.stream().filter(l -> l.equalsThingUID(thingUID))
                .findFirst();
