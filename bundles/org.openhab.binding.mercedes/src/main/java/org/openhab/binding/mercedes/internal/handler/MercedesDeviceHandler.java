...
/**
 * The {@link MercedesDeviceHandler} is responsible for handling commands, which are sent to one of the channels.
 *
 * @author Andreas Stenlund - Initial contribution
 * @author Hilbrand Bouwkamp - Code cleanup, moved channel state to this class, generic stability.
 */
@NonNullByDefault
public class MercedesDeviceHandler extends BaseThingHandler {

    private final Logger logger = LoggerFactory.getLogger(MercedesDeviceHandler.class);
    private @NonNullByDefault({}) MercedesHandleCommands commandHandler;
     ...
     */
    public MercedesDeviceHandler(Thing thing) {
        super(thing);
    }
    ...
    public void initialize() {
        final MercedesBridgeHandler bridgeHandler = (MercedesBridgeHandler) getBridge().getHandler();
        mercedesApi = bridgeHandler.getMercedesApi();

        if (mercedesApi == null) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, String.format(
                    "Missing configuration from the Mercedes Bridge (UID:%s). Fix configuration or report if this problem remains.",
        ...
        if (deviceName == null || deviceName.isEmpty()) {
            ...
            commandHandler = new MercedesHandleCommands(mercedesApi);
    ...
    public void bridgeStatusChanged(ThingStatusInfo bridgeStatusInfo) {
        if (bridgeStatusInfo.getStatus() != ThingStatus.ONLINE) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.BRIDGE_OFFLINE, "Mercedes Bridge Offline");
            logger.debug("MercedesDevice {}: MercedesBridge is not online: {}", getThing().getThingTypeUID(),
    ...
    public void setStatusGone() {
        if (getThing().getStatus() != ThingStatus.OFFLINE
            ...
            getThing().setStatusInfo(new ThingStatusInfo(ThingStatus.OFFLINE, ThingStatusDetail.GONE,
                    "Device not available on Mercedes"));
